...
const order = (state = initialState, action) => {
  switch (action.type) {
      ...
      const productList =
        action.payload.data.status === 200
          ? action.payload.data.result.data.map(item => ({
              ...item,
              isSelected: state.detailOrder.some(
                row => row.prod_id === item.id,
              ),
            }))
          : [];
      return {
    ...
    case 'ADD_ITEM_IN_ORDER':
      // const detailOrder = state.detailOrder.slice(0);
      // detailOrder.push({
    ...
    case 'REMOVE_ITEM_IN_ORDER':
      ...
      // const product = state.productList.slice(0);
      ...
      console.log(afterEditRemove, 'afterEditRemove');
